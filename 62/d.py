areaColorCatalog = {
    "green": {"min": 0, "max": 10},   # Using a reasonable range for green
    "blue": {"min": 11, "max": 30},   # Updated ranges
    "red": {"min": 31, "max": None}
}
...
for color, limits in areaColorCatalog.items():
    if (limits["min"] <= wallArea) and (limits["max"] is None or wallArea <= limits["max"]):
        wallColor = color
        break

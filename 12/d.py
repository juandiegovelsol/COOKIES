def should_dealer_hit(hand, hit_soft_17=False):
    ...
    for card in hand:
        if card in ['J', 'Q', 'K']:
            ...
            values.append(11)  # Use 11 initially for Ace
        else:
            values.append(int(card))
    
    total = sum(values)
    aces = values.count(11)  # Count Aces that are currently valued as 11

    # Adjust total if total is over 21 and there are Aces counted as 11
    while total > 21 and aces > 0:
        total -= 10  # Adjust by treating one Ace as 1
        aces -= 1
    
    # Check for soft 17 (Ace + 6)
    is_soft_17 = (total == 17 and aces > 0)

    if total <= 16:
        return True
    if is_soft_17 and hit_soft_17:
        return True
